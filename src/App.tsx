...
function Board({ boardDef, panelState, setPanelState, hidePersonal, allCardsRef, isMobile }: any) {
  ...
  const renderCard = (card: any) => {
    ...
    return (
      <div key={card.id}
        draggable
        onDragStart={(e: any)=>{ e.dataTransfer.effectAllowed="move"; gDrag.cardId=card.id; gDrag.boardId=boardDef.id; gDrag.card=card; setTick((t: number)=>t+1); }}
        onDragEnd={()=>{ gDrag.cardId=null; gDrag.boardId=null; gDrag.card=null; setDragOverCol(null); setTick((t: number)=>t+1); }}
